...
impl QueryOptions {
    ...
    /// Controls whether the server uses the collection default consistency.
    pub fn use_default_consistency(mut self, enabled: bool) -> Self {
        if enabled {
...
pub struct SearchOptions {
    ...
    pub(crate) consistency_level: Option<i32>,
...
impl SearchOptions {
    ...
    /// Creates SearchOptions with specified consistency level
    pub fn with_consistency_level(consistency_level: i32) -> Self {
        Self::default().consistency_level(consistency_level)
    }
    ...
    pub fn add_param(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        ...
        if key == "consistency_level" {
            if let Ok(parsed) = value.parse::<i32>() {
                self.consistency_level = Some(parsed);
            }
        }
        self.search_params.push(KeyValuePair { key, value });
        self
    }
...
impl Client {
        ...
        S: Into<String>,
    {
        ...
        let use_default_consistency = crate::v1::collection::request_uses_default_consistency(
            &search_params,
            options.consistency_level,
        );
        ...
        S: Into<String>,
    {
        ...
        for req in reqs {
            ...
            let use_default_consistency =
                crate::v1::collection::request_uses_default_consistency(&search_params, None);
...
fn get_place_holder_value(vectors: &Vec<Value>) -> Result<PlaceholderValue> {
    ...
    match vectors[0] {
        ...
        Value::SparseFloat(_) => place_holder.r#type = PlaceholderType::SparseFloatVector as _,
