...
use crate::v1::client::{Client, CombinedInterceptor, DbInterceptor};
...
impl Client {
    ...
    pub async fn using_database<S: Into<String>>(&mut self, db_name: S) -> Result<()> {
        ...
        // Clear only schema entries for the target database so they refresh on first use.
        self.collection_cache.clear_db(&db_name);
        ...
        self.collection_cache = CollectionCache::with_shared_state(
            new_client,
            self.collection_cache.shared_collections(),
            self.collection_cache.shared_timestamps(),
        );
