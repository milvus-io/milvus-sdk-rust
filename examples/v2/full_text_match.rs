...
async fn main() -> Result<()> {
    const COLLECTION: &str = "RUST_V2_FULL_TEXT_MATCH";
    ...
    for text in ["moon and earth distance", "Milvus vector database"] {
        println!("================================================================");
        ...
        let response = client
            .search(
                SearchRequest::builder()
                    .collection_name(COLLECTION)
                    .vector_field(VECTOR)
                    .vectors(SearchVectors::EmbeddedText(vec![text.into()]))
